...
uint16_t CPU::readWord(uint16_t address) {
  uint16_t adl = (uint16_t)this->ram->read(address);
  uint16_t adh = (uint16_t)this->ram->read(address + 1);
  return (adh << 8) | adl;
}

/*
  There's a bug in 6502 where the low byte is
  wrapped without incrementing the high byte
*/
uint16_t CPU::readWord_buggy(uint16_t address) {
